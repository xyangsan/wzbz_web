// 根据阳历获取阴历
function getSolarDateData(dateStr) {
    let arr = dateStr.split("年");
    let year = parseInt(arr[0]);
    let yearindex = parseInt((year - 1801) / 5);
    console.log("yearindex: " + yearindex)
    let dateDic = initDateDataWithIndex(yearindex);
    let solarDateMap = dateDic["solarDate"];
    console.log(solarDateMap)
    console.log("dateStr: " + dateStr)
    let lunarStr = solarDateMap.get(dateStr);
    console.log("根据阳历获取阴历：" + lunarStr)
    return lunarStr;
}

function getLunarDateData(dateStr) {
    let arr = dateStr.split("年");
    let year = Integer.parseInt(arr[0]);
    let yearindex = parseInt((year - 1801) / 5);
    console.log("yearindex: " + yearindex)
    let dateDic = initDateDataWithIndex(yearindex);
    let lunarDateMap = dateDic["lunarDate"];
    console.log(lunarDateMap)
    console.log("dateStr: " + dateStr)
    let solarStr = lunarDateMap.get(dateStr);
    console.log("根据阴历获取阳历：" + solarStr)
    return solarStr;
}

function getDateDataWithDate(year, month, day) {
    let paipanUtils = new paipan();
    var fm = paipanUtils.fatemaps(0, year, month, day, 0, 0, 0); //不考虑真太阳时
    var nongliYear = fm.nl[0];
    let nongliDate = nongliYear + "年" + (fm.nl[3]?'闰':'') + paipanUtils.dxy[fm.nl[1]-1] + paipanUtils.dxd[fm.nl[2]-1];
    let sizhuArr = fm.sz;
    let weekday = "星期" + paipanUtils.wkd[paipanUtils.GetWeek(year, month , day)];
    let dateDic = {
