function rlDate() {
     ...
     * 廿四节气(从春分开始) JieQi
     */
    this.jq = ['春分', '清明', '谷雨', '立夏', '小满', '芒种', '夏至', '小暑', '大暑', '立秋', '处暑', '白露', '秋分', '寒露', '霜降', '立冬', '小雪',
     ...
     * 大写日期
     */
    this.dxd = ['初一', '初二', '初三', '初四', '初五', '初六', '初七', '初八', '初九', '初十', '十一', '十二', '十三', '十四', '十五', '十六', '十七',
    ...
    this.VE = function (yy) {
        ...
        if (yx >= 1000 && yx <= 8001) {
            var m = (yx - 2000) / 1000;
            var jdve = 2451623.80984 + 365242.37404 * m + 0.05169 * m * m - 0.00411 * m * m * m - 0.00057 * m * m *
                m * m;
        } else {
            if (yx >= -8000 && yx < 1000) {
                var m = yx / 1000;
                var jdve = 1721139.29189 + 365242.1374 * m + 0.06134 * m * m + 0.00111 * m * m * m - 0.00071 * m *
                    m * m * m;
            } else { //超出计算能力范围
                return false;
            }
    ...
    this.MeanJQJD = function (yy) {
        ...
        var e = 0.0167086342 - 0.0004203654 * tx - 0.0000126734 * tx * tx + 0.0000001444 * tx * tx * tx -
            0.0000000002 * tx * tx * tx * tx + 0.0000000003 * tx * tx * tx * tx * tx;
        var tt = yy / 1000;
        var vp = 111.25586939 - 17.0119934518333 * tt - 0.044091890166673 * tt * tt - 4.37356166661345E-04 * tt *
            tt * tt + 8.16716666602386E-06 * tt * tt * tt * tt;
    ...
    this.Perturbation = function (jdez) {
        ...
        var ptsa = [485, 203, 199, 182, 156, 136, 77, 74, 70, 58, 52, 50, 45, 44, 29, 18, 17, 16, 14, 12, 12, 12, 9,
            8
        ];
        var ptsb = [324.96, 337.23, 342.08, 27.85, 73.14, 171.52, 222.54, 296.72, 243.58, 119.81, 297.17, 21.02,
            247.54, 325.15, 60.93, 155.12, 288.79, 198.04, 199.76, 95.39, 287.11, 320.81, 227.73, 15.45
        ];
        var ptsc = [1934.136, 32964.467, 20.186, 445267.112, 45036.886, 22518.443, 65928.934, 3034.906, 9037.513,
    ...
    this.DeltaT = function (yy, mm) {
        ...
        if (y <= -500) {
            ...
            if (y < 500) {
                ...
                if (y < 1600) {
                    u = (y - 1000) / 100;
                    dt = (1574.2 - 556.01 * u + 71.23472 * u * u + 0.319781 * u * u * u - 0.8503463 * u * u * u *
                        u - 0.005050998 * u * u * u * u * u + 0.0083572073 * u * u * u * u * u * u);
                } else {
                    if (y < 1700) {
                        t = y - 1600;
                        ...
                        if (y < 1800) {
                            t = y - 1700;
                            dt = (8.83 + 0.1603 * t - 0.0059285 * t * t + 0.00013336 * t * t * t - t * t * t * t /
                                1174000);
                        } else {
                            if (y < 1860) {
                                t = y - 1800;
                                dt = (13.72 - 0.332447 * t + 0.0068612 * t * t + 0.0041116 * t * t * t -
                                    ...
                                    0.0000001699 * t * t * t * t * t * t + 0.000000000875 * t * t * t * t * t *
                                    t * t);
                            } else {
                                if (y < 1900) {
                                    t = y - 1860;
                                        ...
                                        if (y < 1941) {
                                            ...
                                            if (y < 1961) {
                                                ...
                                                if (y < 1986) {
                                                    ...
                                                    if (y < 2005) {
                                                        t = y - 2000;
                                                        dt = (63.86 + 0.3345 * t - 0.060374 * t * t + 0.0017275 *
                                                            t * t * t + 0.000651814 * t * t * t * t +
    ...
    this.GetAdjustedJQ = function (yy, calendar) {
        ...
        if (this.JQ[yy][0] == undefined) { //必须要有0
            ...
            for (var i = 0; i < 24; i++) {
                ...
                jdjq[i] = jdez[i] + ptb - dt / 60 /
                    24; //加上攝動調整值ptb得到動態時間dynamical time or ephemeris days,減去對應的Delta T值(分鐘轉換為日)得到True Universal time
        ...
        if (calendar == 1) { //古代农历模块完全来自寿星万年历
            ...
            var f1 = B[0] -
                pc; //1640650.479938 - 7, -221-11-09 h=0.01709 古历·秦汉开始至没有天文算法(定气定朔)之前,古人采取平气平朔的方法,各朝代改动较大,许老师从繁杂的古历中提取数据进行分析而得到算法,致敬!
     ...
     * 获取某一年的节气时间，从小寒开始
     * @param {*} yy  
     * @param {*} type 0:小寒开始 1:春风开始
     */
    this.getJQByArr = function (yy, type = 0) {
        //小寒-大寒-立春-雨水-惊蛰-春分 0
        //立春-雨水-惊蛰-春分 1
        let y = this.intval(yy)
        let jqArr1 = this.GetAdjustedJQ(y - 1)
        let jqArr2 = this.GetAdjustedJQ(y)
        let res = []
        const startIdx = (type === 0) ? 19 : 21;
        for (var i = startIdx; i < 24; i++) {
            res.push(this.Jtime(jqArr1[i]))
        }
        for (var i = 0; i < startIdx; i++) {
            res.push(this.Jtime(jqArr2[i]))
        }
    ...
    this.MeanNewMoon = function (jd, return_k) {
        ...
        var k = Math.floor((jd - 2451550.09765) / this
            .synmonth); //2451550.09765為2000年1月6日14時20分36秒之JD值,此為2000年後的第一個均值新月
        if (return_k) {
            return k;
        }
        ...
        var pt = jdt + 0.0001337 * t * t - 0.00000015 * t * t * t + 0.00000000073 * t * t * t *
            t; //mean time of phase
    ...
    this.GetZQandSMandLunarMonthCode = function (yy) {
        ...
        if (Math.floor(qjd[12] + 0.5) >= Math.floor(sjd[13] + 0.5)) { //若第13個中氣jdzq(12)大於或等於第14個新月jdnm(13)
            for (var i = 1; i <= 14; i++) { //表示此兩個冬至之間的11個中氣要放到12個朔望月中,
                ...
                if (Math.floor((sjd[i] + 0.5) > Math.floor(qjd[i - 1 - yz] + 0.5) && Math.floor(sjd[i + 1] + 0.5) <=
                    Math.floor(qjd[i - yz] + 0.5))) {
                    mc[i] = i - 0.5;
                    yz = 1; //標示遇到閏月
                } else {
    ...
    this.Lunar2Solar = function (yy, mm, dd, ry) { //此為將陰曆日期轉換為陽曆日期的主程式
        ...
        ob.cnm = this.dxy[mm - 1]
        if (ry) ob.cnm = '闰' + ob.cnm
        ob.cnd = this.dxd[dd - 1]
        return [yi, mi, dz, ob];
    };
    ...
    this.Solar2Lunar = function (yy, mm, dd) {
        ...
        if (jdx >= 1457698 && jdx <= 1683430) { //这一段来自寿星万年历,十九年七闰法:7个闰年均匀安插在19个年整数中,闰年的末月置为闰月
            ...
            ob.cnm = this.dxy[(mi + ns[ii + 11] - 2) % 12];
            ob.days = tjd[mi] - tjd[mi - 1]; //该月多少天

            if (tjd.length == 14) { //该年有闰月
                if (mi == 13) { //都是最后一个月置闰
                    ry = true;
                    ob.cnm = ns[ii + 6]; //非常规,可能史书上称后九或十三
                }
            }
        ...
        if (Dm == 1729794 || Dm == 1808699) {
            ob.cnm = '拾贰'; //239.12.13及23.12.02均为十二月,为避免两个连续十二月，此处改名
        }
        if (ry) ob.cnm = "闰" + ob.cnm
        ob.cnd = this.toChinaDay(dz)
    ...
    this.toChinaDay = function (day) {
        var day = this.intval(day);
        return this.dxd[day - 1];
    };
     ...
     * @param {*} isFormat 
     */
    this.getGZTime = function (gzDay, hour, isFormat = true) {
        if (hour == 0 && isFormat) return '**'
        let dzIndex = Math.round(hour / 2) % 12
        let tgDay = gzDay.split("")[0]
        let tzIndex = this.ctg.indexOf(tgDay) % 5
        return this.ctg[(tzIndex * 2 + dzIndex) % 10] + this.cdz[dzIndex]
    };
}

export const solar2Lunar = (...args) => {
	return new rlDate().Solar2Lunar(...args)
}

export const lunar2Solar = (yy, mm, dd, ry) => {
    return new rlDate().Lunar2Solar(yy, mm, dd, ry)
}

export const getLeap = (yy) => {
    return new rlDate().GetLeap(yy)
}